...
final class CarTripItemViewModel : ItemViewModelType {
    ...
    init(model: CarTrip) {
        ...
        dateFormatter.locale = Locale(identifier:Config().langAndCountry)
        dateFormatter.timeZone = TimeZone(identifier: "Europe/Rome")
        let timeFormatter = DateFormatter()
        timeFormatter.dateFormat = "lbl_carTripsTimeFormatter".localized()
        timeFormatter.locale = Locale(identifier: Config().langAndCountry)
...
//            }
            ...
            self.description.value = String(format: "lbl_carTripsItemExtendedDescription".localized(), startDateText.uppercased(), startTimeText.uppercased(), startAddress, endDateText.uppercased(), endTimeText.uppercased(), endAddress, plate).replacingOccurrences(of: ".", with: ",").replacingOccurrences(of: ",00", with: "")
