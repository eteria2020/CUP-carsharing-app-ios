...
public class CarAnnotation: NSObject, GMUClusterItem {
    ...
    public func freeImage(image: UIImage, value: Int) -> (UIImage) {
        ...
        let attrs = [NSAttributedString.Key.font: UIFont(name: "Poppins-SemiBold", size: 25)!, NSAttributedString.Key.foregroundColor: UIColor.white, NSAttributedString.Key.strokeWidth: -6.0,
                     NSAttributedString.Key.strokeColor: ColorBrand.green.value,NSAttributedString.Key.paragraphStyle: paragraphStyle] as [NSAttributedString.Key : Any]
        let string = "\(value)"
