...
public extension ObservableType where E == Moya.Response {
  ...
  public func mapJSONObject<T: JSONDecodable>(type: T.Type) -> Observable<T> {
  ...
  /// Observable .Errors's on failure.
  public func mapJSONObject<T: JSONDecodable>(type: T.Type, forKeyPath keyPath: String) -> Observable<T> {
    return flatMap { response -> Observable<T> in
  ...
  /// Observable .Errors's on failure.
  public func mapJSONArray<T: JSONDecodable>(type: T.Type) -> Observable<[T]> {
    return flatMap { (response) -> Observable<[T]> in
  ...
  /// Observable .Errors's on failure.
  public func mapJSONArray<T: JSONDecodable>(type: T.Type, forKeyPath keyPath: String) -> Observable<[T]> {
    return flatMap { (response) -> Observable<[T]> in
